...
if (domainName == "reg-prod.ec.jccc.edu") {
  async function mainAsync() {
    ...
    let response = await fetch(
      "https://jccc-class-formatter.bluerose.garden/bookmarklet-ver.json",
    );
    ...
    if (confirmation && versionConfirmation) {
      ...
      let timeRE =
        /\d\d:\d\d  (?:AM|PM) - \d\d:\d\d  (?:AM|PM) (?!Type: Exam)/g;
      ...
      for (var i = 1; i < rows.length; i++) {
        ...
        for (var j = 0; j < cols.length; j++) {
          ...
          if (cols[j].dataset.property == "courseTitle") {
            ...
            let profName = profNameArr?.length > 0
              ? `${profNameArr[2].trim()} ${profNameArr[1].trim()} ${
                profNameArr[3].trim()
              }`
              : "N/A";
            ...
            classInfo.location = room
              ? `${classInfo.location} ${room[0]}`
              : "N/A";
            classInfo.startend = startend ? startend : "N/A";

            //If hybrid, list when online section starts
            if (
              classInfo.delivery === "Online Hybrid" ||
              classInfo.delivery === "Hybrid"
            ) {
              let startendSecond = startendRE.exec(cols[j].title);
              classInfo.startend =
                `${classInfo.startend} and the online section begins ${startendSecond}`;
            }
            ...
            classInfo.waitlist = waitlistArr
              ? `${waitlistArr[1]}/${waitlistArr[2]}`
              : "N/A";
        ...
        csv_data.push(
          `${classInfo.title.replaceAll(",", " ")}, ${
            classInfo.subj.replaceAll(",", " ")
          }, ${classInfo.crn.replaceAll(",", " ")}, ${
            classInfo.hours.replaceAll(",", " ")
          },${classInfo.instructor.replaceAll(",", " ")}, ${
            classInfo.email.replaceAll(",", " ")
          }, ${classInfo.delivery.replace(",", " ")}, ${
            classInfo.meetingTimes.replaceAll(",", " ")
          }, ${
            classInfo.location.replaceAll(",", " ")
          }, ${classInfo.startend}, ${classInfo.seats}, ${classInfo.waitlist}, ${
            classInfo.attributes.replaceAll(",", " ")
          }`,
        );
      }
