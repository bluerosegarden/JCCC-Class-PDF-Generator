...
//Processes all the files uploaded
async function processFiles() {
  ...
  if (fileList.length >= 1) {
    for (let i = 0; i < await fileList.length; i++) {
      console.log(`${i} loop`);

      let uploadedFile = await fileList[i];
      ...
      if (fileExt.toLowerCase() != "csv") {
        alert("One or more of the files are not CSV!");
        break;
      }
...
async function copyLinkToCLipboard() {
  ...
  const minifiedJSResponse = await fetch(
    "../bookmarklet/bookmarklet-minified.js",
  );
...
async function formatTemplate(csvFile) {
  if (groupedCheckbox.checked) {
    const grouped_response = await fetch(
      "../templates/grouped-paper-classes.typ",
    );
    ...
    ungrouped_text = ungrouped_text.replace(
      '"input.tmp.csv"',
      `bytes("${csvFile}")`,
    );
